...
export function WordMatchingGameComponent(props: WordButtonProps) {
  ...
  }, [selectedEnglish, selectedChinese, gameWords, words])
  ...
  useEffect(() => {
    if (words.length > 0) {
      const progressValue = (matchedWords.length / words.length) * 100
  ...
  return (
    <div className="container mx-auto p-4">
      ...
      <div className="fixed top-0 left-0 right-0 bg-white/95 backdrop-blur-sm shadow-lg z-50 p-4">
        ...
        <div className="flex justify-between items-center mb-2">
          ...
          <div className="text-xl">时间:
            <span className={`${timeLeft < 10 ? 'text-red-500' : ''} w-16 inline-block text-center`}>
              {timeLeft}
            </span>s
          </div>
        </div>
        ...
        {!gameStarted ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 relative">
            <div className="relative">
              <div className="text-lg font-semibold mb-4 text-gray-700">English Words</div>
                ...
                {displayWords.english.map((word, index) => (
                  <motion.div
                    ...
                    animate={{
                      ...
                      y: fadingOutWords.some(w => w.english === word.english) ? -20 : 0,
                      rotate: fadingOutWords.some(w => w.english === word.english) ? -10 : 0,
                    }}
                  ...
                  >
                    <ButtonWrapper
                      ...
                      className={`w-full py-6 transition-all duration-100 transform hover:scale-105 
                        ${selectedEnglish === word.english ? 'bg-blue-600 hover:bg-blue-500 shadow-lg' : 'hover:shadow-md'}
                        ${matchedPair?.english === word.english ? 'bg-green-600' : ''}
                        ${mismatchedPair?.english === word.english ? 'bg-red-400' : ''}`}
                      whileHover={{ y: -5 }}
                      whileTap={{ scale: 0.95 }}
                      animate={mismatchedPair?.english === word.english ? {
                        x: [-8, 8, -8, 8, -4, 4, 0],
            ...
            <div className="relative">
              <div className="text-lg font-semibold mb-4 text-gray-700">中文释义</div>
              <div className="space-y-4">
                {displayWords.chinese.map((word, index) => (
                  <motion.div
                    ...
                    animate={{
                      ...
                      y: fadingOutWords.some(w => w.chinese === word.chinese) ? -20 : 0,
                      rotate: fadingOutWords.some(w => w.chinese === word.chinese) ? 10 : 0,
                    }}
                    transition={{ duration: 0.4, type: "spring" }}
                    onAnimationComplete={() => {
                  ...
                  >
                    <ButtonWrapper
                      ...
                      className={`w-full py-6 transition-all duration-100 transform hover:scale-105
                        ${selectedChinese === word.chinese ? 'bg-blue-500 hover:bg-blue-400 shadow-lg' : 'hover:shadow-md'}
                        ${matchedPair?.chinese === word.chinese ? 'bg-green-500' : ''}
                        ${mismatchedPair?.chinese === word.chinese ? 'bg-red-400' : ''}`}
                      whileHover={{ y: -5 }}
                      whileTap={{ scale: 0.95 }}
                      animate={mismatchedPair?.chinese === word.chinese ? {
                        x: [-8, 8, -8, 8, -4, 4, 0],
                        transition: { duration: 0.2, type: "spring", stiffness: 400 }
                      } : {}}
                    >
            ...
            <div className="absolute left-1/2 top-0 bottom-0 w-px bg-gradient-to-b from-transparent via-gray-300 to-transparent transform -translate-x-1/2 hidden md:block" />
          </div>
        )}
      </div>
    </div>
